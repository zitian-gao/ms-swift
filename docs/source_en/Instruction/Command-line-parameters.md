...
- eval_datasets_dir: Local directory where EvalScope benchmark datasets are cached. Useful for offline environments: pre-download datasets on a machine with internet access and set this to that directory. Defaults to `~/.cache/modelscope/hub/datasets`
